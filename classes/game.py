...
class Game:
    ...
    def run_pygame(self):
        ...
        pygame.key.set_repeat(100, 100)  # Moving player while long pressing on keys
        ...
        while running:
            ...
            while self.game_over:
                self.game_started = False
                if self.good_game == True:
                    self.window.blit(pygame.image.load(config.WIN_PIC).convert(),
                                     (0, 0))
                    pygame.display.flip()

                if self.failed_game is True:
                    self.window.blit(pygame.image.load(config.LOSE_PIC).convert_alpha(),
                                     (0, 30))
                    pygame.display.flip()
